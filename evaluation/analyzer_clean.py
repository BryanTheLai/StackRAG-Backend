...
class ResultAnalyzer:
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if failure_counts.get('Number Accuracy', 0) > 0:
            recommendations.append(
                f"🔢 Fix Number Accuracy ({failure_counts['Number Accuracy']} failures): "
                "Improve number extraction and scaling logic. Ensure proper conversion from "
                "thousands to actual dollar amounts."
            )
        
        # Answer correctness issues
        if failure_counts.get('Answer Correctness', 0) > 0:
            recommendations.append(
                f"📝 Improve Answer Quality ({failure_counts['Answer Correctness']} failures): "
                "Ensure responses fully address the user's question. Add validation that "
                "required information is included."
            )
        
        # Faithfulness issues
        if failure_counts.get('Faithfulness', 0) > 0:
            recommendations.append(
                f"🎯 Fix Faithfulness ({failure_counts['Faithfulness']} failures): "
                "Improve context grounding. Ensure responses only use information from "
                "retrieved documents without hallucination."
            )
        
        # RAG success issues
        if failure_counts.get('RAG Success', 0) > 0:
            recommendations.append(
                f"🚀 Fix RAG Pipeline ({failure_counts['RAG Success']} failures): "
                "Improve retrieval quality and response generation. Check document "
        ...
        if metrics['avg_execution_time'] > 30:
            recommendations.append(
                f"⚡ Optimize Performance: Average execution time is {metrics['avg_execution_time']:.1f}s. "
                "Consider optimizing retrieval and generation speed."
            )
